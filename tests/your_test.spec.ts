...
import { LeaveStartPage } from "./pages/leaveTestPage";
...
test(`Your test - Task 1 - flow diagram 2`, async ({ page }): Promise<void> => {
  ...
  const entitlementBasedOnPage: EntitlementBasedOnPage =
    new EntitlementBasedOnPage(page);
  ...
  const daysWorkedPerWeekPage: DaysWorkedPerWeekPage =
    new DaysWorkedPerWeekPage(page);
  ...
  await summaryPage.expectSummary(
